...
const StudentProfile = () => {
  ...
  useEffect(() => {
    if (student) setProfile({
      name: student.name, course: student.course, yearLevel: student.yearLevel, profilePic: student.profilePic || ""
    });
  }, [student]);
  ...
  const handleProfilePicUpload = async (e) => {
    ...
    try {
      ...
      const res = await axios.post("/api/students-profile/profile/upload", formData, {
  ...
  return (
    <Container sx={{ mt: 8, display: "flex", justifyContent: "center" }}>
      <Card sx={{ width: 500, borderRadius: 4, boxShadow: 6 }}>
        <Box sx={{ background: "linear-gradient(135deg, #42a5f5, #1976d2)", p: 4, color: "#fff", display: "flex", flexDirection: "column", alignItems: "center" }}>
          <Box position="relative">
            <Avatar src={profile.profilePic} sx={{ width: 100, height: 100, fontSize: 36, bgcolor: "#fff", color: "#1976d2", border: "3px solid #fff" }}>
              {!profile.profilePic && (profile.name?.[0] || user?.email?.[0])?.toUpperCase()}
            </Avatar>
            <IconButton component="label" sx={{ position: "absolute", bottom: -5, right: -5, bgcolor: "#1976d2", color: "#fff" }} disabled={uploading}>
        ...
        <CardContent>
          <Stack spacing={2}>
            {editMode ? (
              <>
                <TextField label="Full Name" value={profile.name} onChange={(e) => setProfile(p => ({ ...p, name: e.target.value }))} fullWidth />
                <TextField label="Course" value={profile.course} onChange={(e) => setProfile(p => ({ ...p, course: e.target.value }))} fullWidth />
                <TextField label="Year Level" value={profile.yearLevel} onChange={(e) => setProfile(p => ({ ...p, yearLevel: e.target.value }))} fullWidth />
                <Stack direction="row" spacing={2}>
                  <Button variant="contained" onClick={handleSaveProfile} disabled={saving}>{saving ? <CircularProgress size={20} color="inherit" /> : "Save"}</Button>
